...
import com.akshay.harsoda.android.base.helper.base.shared_prefs.BaseConfig
import com.akshay.harsoda.android.base.helper.base.utils.isTiramisuPlus
import com.akshay.harsoda.android.base.helper.base.utils.makeText
...
//    val mDefaultCoinDialog: DefaultCoinDialog by lazy { DefaultCoinDialog(activity = mActivity) }
//
//    val mAppUpdateDialog: AppUpdateDialog by lazy {
//        AppUpdateDialog(
//            activity = mActivity,
////            iconID = R.drawable.splsh_icon,
//            iconID = R.drawable.ic_launcher_background,
//            onUpdateClick = {
//                Launcher.openGooglePlay(context = mActivity, packageName = mActivity.packageName)
//            },
//            onCancelClick = {
...
//    val mRatingDialog: RatingDialog by lazy {
//        RatingDialog(
//            activity = mActivity
//        )
//    }
...
//    private val mNoInternetDialog: NoInternetDialog by lazy {
//        NoInternetDialog(
//            activity = mActivity,
//            onShow = {
//                onNoInternetDialogShow()
//            },
//            onDismiss = {
//                onNoInternetDialogDismiss()
//            }
//        )
//    }
...
//        if (isOnline) {
//            initAds()
//            InterstitialAdHelper.loadAd(mActivity)
//        }
    }
    //</editor-fold>

    //<editor-fold desc="New Activity Intent">
    open fun getActivityIntent(cls: Class<*>): Intent {
        return mActivity.getActivityIntent(cls = cls, isAddFlag = true)
    }

    open fun getActivityIntent(
        cls: Class<*>,
        isAddFlag: Boolean = true
    ): Intent {
        return this.getActivityIntent(cls = cls, isAddFlag = isAddFlag, fBundle = {})
    }
    ...
    open fun launchActivityForResult(fIntent: Intent, onActivityResult: (resultCode: Int, data: Intent?) -> Unit) {
        launchActivityForResult(fIntent = fIntent, isAdsShowing = true, onActivityResult = onActivityResult)
    }

    open fun launchActivityForResult(fIntent: Intent, isAdsShowing: Boolean = true, onActivityResult: (resultCode: Int, data: Intent?) -> Unit) {
        launchActivityForResult(
            fIntent = fIntent,
            isAdsShowing = isAdsShowing,
            isCheckResolveActivity = false,
            onActivityResult = onActivityResult
        )
    }

    open fun launchActivityForResult(cls: Class<*>, onActivityResult: (resultCode: Int, data: Intent?) -> Unit) {
        launchActivityForResult(cls = cls, isAdsShowing = true, onActivityResult = onActivityResult)
    }

    open fun launchActivityForResult(cls: Class<*>, isAdsShowing: Boolean = true, onActivityResult: (resultCode: Int, data: Intent?) -> Unit) {
        launchActivityForResult(
            fIntent = getActivityIntent(cls),
            isAdsShowing = isAdsShowing,
            onActivityResult = onActivityResult
        )
    }

    open fun launchActivityForResult(cls: Class<*>, fBundle: Bundle.() -> Unit = {}, onActivityResult: (resultCode: Int, data: Intent?) -> Unit) {
        launchActivityForResult(cls = cls, isAdsShowing = true, fBundle = fBundle, onActivityResult = onActivityResult)
    }

    open fun launchActivityForResult(cls: Class<*>, isAdsShowing: Boolean = true, fBundle: Bundle.() -> Unit = {}, onActivityResult: (resultCode: Int, data: Intent?) -> Unit) {
        launchActivityForResult(
            fIntent = getActivityIntent(cls, true, fBundle),
            isAdsShowing = isAdsShowing,
            onActivityResult = onActivityResult
        )
    }
    ...
    private fun launchActivityForResult(
        fIntent: Intent,
        isCheckResolveActivity: Boolean = false,
...
//        if (!mNoInternetDialog.isShowing) {
//            if (isAdsShowing) {
//                mActivity.showInterstitialAd(isBackAds = false) {
//                    launchActivityForResult(
//                        fIntent = fIntent,
...
//        }
    ...
    open fun launchActivity(fIntent: Intent) {
        launchActivity(fIntent = fIntent, isAdsShowing = true)
    }
    ...
    open fun launchActivity(cls: Class<*>) {
        launchActivity(cls = cls, isAdsShowing = true)
    }

    open fun launchActivity(cls: Class<*>, isAdsShowing: Boolean = true) {
        launchActivity(
            fIntent = getActivityIntent(cls),
            isAdsShowing = isAdsShowing
        )
    ...
    open fun launchActivity(cls: Class<*>, fBundle: Bundle.() -> Unit = {}) {
        launchActivity(cls = cls, isAdsShowing = true, fBundle = fBundle)
    }

    open fun launchActivity(cls: Class<*>, isAdsShowing: Boolean = true, fBundle: Bundle.() -> Unit = {}) {
        launchActivity(
            fIntent = getActivityIntent(cls, true, fBundle),
            isAdsShowing = isAdsShowing
        )
    ...
    open fun launchActivity(
        ...
        launchActivity(
            ...
            fEnterAnimId = android.R.anim.fade_in,
            fExitAnimId = android.R.anim.fade_out
        )
    }
...
//            mActivity.showInterstitialAd(isBackAds = true) {
//                directBack()
//            }
//        } else {
            directBack()
